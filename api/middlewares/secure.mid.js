...
const {Restaurant, Review, Product, Zone, Table, Schedule, ScheduleHours, RestaurantSetting, Timeslot, Booking} = require('../models');
...
module.exports.isScheduleHourOwnedByUser = (req, res, next) => {
  const { id } = req.params;

  ScheduleHours.findById(id)
    .then((scheduleHour) => {
      checkUser(scheduleHour, 'scheduleHour', req, next)
    })
    .catch(next);
};
...
module.exports.isRestaurantSettingsOwnedByUser = (req, res, next) => {
  const { id } = req.params;
...
module.exports.isBookingOwnedByUser = (req, res, next) => {
  const { id } = req.params;

  Booking.findById(id)
    .then((booking) => {
      checkUser(booking, 'booking', req, next)
    })
    .catch(next);
};